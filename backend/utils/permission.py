...
async def require_verified_user(current_user: dict = Depends(get_current_user)) -> dict:
    ...
    if user.get("banned"):
        raise HTTPException(
            ...
            detail="Account is banned. You cannot perform this action."
        )
...
async def require_moderator_user(current_user: dict = Depends(get_current_user)) -> dict:
    """要求当前用户为 moderator 或 admin"""
    db = get_database()
    user = await db.users.find_one({"_id": ObjectId(current_user["user_id"])})
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
